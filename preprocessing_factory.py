...
def get_preprocessing(name, is_training=False):
    ...
    def unprocessing_fn(image, **kwargs):
        return unprocess_image(image, **kwargs)
...
def _smallest_size_at_least(height, width, target_height, target_width):
    ...
    """
    target_height = tf.to_float(tf.convert_to_tensor(target_height, dtype=tf.int32))
    target_width = tf.to_float(tf.convert_to_tensor(target_width, dtype=tf.int32))
